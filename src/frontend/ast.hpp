...
struct LogicalExpr : Expr {
    LogicalExpr(std::unique_ptr<Expr> left, const Token& op, std::unique_ptr<Expr> right)
        : left(std::move(left)), op(op), right(std::move(right)) {
        location = this->left->location;
        location.end_line = this->right->location.end_line;
        location.end_column = this->right->location.end_column;
...
struct SetExpr : Expr {
    SetExpr(std::unique_ptr<Expr> object, const Token& name, std::unique_ptr<Expr> value)
...
struct ForStmt : Stmt {
    ForStmt(const Token& variable, std::unique_ptr<Expr> iterable, std::unique_ptr<Stmt> body)
...
struct ClassStmt : Stmt {
    ClassStmt(const Token& name, std::vector<std::unique_ptr<FunctionStmt>> methods)
